...
export async function depositTornado(
  contractAddress: string,
...
export async function withdrawTornado(
  contractAddress: string,
  proof: any,
  args: any,
  wallet: ethers.Signer
  ...
  const withdrawTx = await nativeTornadoInstance.withdraw(proof, ...args, {
    gasLimit: 6000000,
  });
...
export async function getDepositLeavesFromRelayer(
  ...
  const relayerResponse = await fetch(
    `${endpoint}/api/v1/leaves/${contractAddress}`
  );
