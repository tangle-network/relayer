...
describe('Cross chain transaction <<>> Mocked Backend', function () {
  ...
  before(async () => {
    ...
    localChain1 = await LocalChain.init({
      ...
      populatedAccounts: [
        {
          secretKey: PK1,
          balance: ethers.utils.parseEther('100000000000000000000000').toHexString(),
        },
        {
          secretKey: GOV,
          balance: ethers.utils.parseEther('100000000000000000000000').toHexString(),
  ...
  it('Evm to Substrate cross chain transaction', async () => {
    ...
    await tx.wait();
    await token.mintTokens(wallet1.address, ethers.utils.parseEther('100000000000000000000000'));
