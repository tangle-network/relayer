...
impl Metrics {
    ...
    pub fn new() -> Self {
        ...
        let bridge_watcher_back_off_counter_opts = Opts::new(
            "bridge_watcher_back_off_metric",
            "specifies how many times the bridge watcher backed off",
        );
        let bridge_watcher_back_off_counter =
            Counter::with_opts(bridge_watcher_back_off_counter_opts).unwrap();

        let handle_proposal_execution_metric_counter_opts = Opts::new(
            "handle_proposal_execution_metric",
            "How many times did the function handle_proposal get executed",
        );
        let handle_proposal_execution_metric_counter =
            Counter::with_opts(handle_proposal_execution_metric_counter_opts)
                .unwrap();

        let transaction_queue_back_off_metric_counter_opts = Opts::new(
            "transaction_queue_back_off_metric",
            "How many times the transaction queue backed off",
        );
        let transaction_queue_back_off_metric_counter =
            Counter::with_opts(transaction_queue_back_off_metric_counter_opts)
                .unwrap();

        let proposal_queue_attempt_metric_counter_opts = Opts::new(
            "proposal_queue_attempt_metric",
            "How many times a proposal is attempted to be queued",
        );
        let proposal_queue_attempt_metric_counter =
            Counter::with_opts(proposal_queue_attempt_metric_counter_opts)
                .unwrap();

        let total_active_relayer_metric_counter_opts = Opts::new(
            "total_active_relayer_metric",
            "The total number of active relayers",
        );
        let total_active_relayer_metric_counter =
        ...
        let total_fee_earned_metric_counter =
            Counter::with_opts(total_fee_earned_metric_counter_opts).unwrap();

        let gas_spent_metric_counter_opts =
            Opts::new("gas_spent_metric", "The total number of gas spent");
        let gas_spent_metric_counter =
            Counter::with_opts(gas_spent_metric_counter_opts).unwrap();

        let total_number_of_proposals_metric_counter_opts = Opts::new(
            "total_number_of_proposals_metric",
            "The total number of proposals proposed",
        );
        let total_number_of_proposals_metric_counter =
            Counter::with_opts(total_number_of_proposals_metric_counter_opts)
                .unwrap();

        let total_number_of_data_stored_metric_counter_opts = Opts::new(
            "total_number_of_data_stored_metric",
            "The Total number of data stored",
        );
        let total_number_of_data_stored_metric_counter =
            Counter::with_opts(total_number_of_data_stored_metric_counter_opts)
                .unwrap();
        ...
        // registers the counters
        registry
            .register(Box::new(bridge_watcher_back_off_counter.clone()))
            .unwrap();
        registry
            .register(Box::new(
                handle_proposal_execution_metric_counter.clone(),
            ))
            .unwrap();
        registry
            .register(Box::new(
                transaction_queue_back_off_metric_counter.clone(),
            ))
            .unwrap();
        registry
            .register(Box::new(proposal_queue_attempt_metric_counter.clone()))
            .unwrap();
        registry
        ...
        registry
            ...
            .unwrap();
        registry
            .register(Box::new(gas_spent_metric_counter.clone()))
            .unwrap();
        registry
            .register(Box::new(
                total_number_of_proposals_metric_counter.clone(),
            ))
            .unwrap();
        registry
            .register(Box::new(
                total_number_of_data_stored_metric_counter.clone(),
            ))
            .unwrap();
        ...
        Self {
            ...
            handle_proposal_execution_metric:
                handle_proposal_execution_metric_counter,
            transaction_queue_back_off_metric:
                transaction_queue_back_off_metric_counter,
            proposal_queue_attempt_metric:
            ...
            total_number_of_proposals_metric:
                total_number_of_proposals_metric_counter,
            total_number_of_data_stored_metric:
