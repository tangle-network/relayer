...
pub trait FinalitySlotHandler {
    ...
    /// be discarded. to have a retry mechanism, use the [`FinalitySlotHandlerWithRetry::handle_event_with_retry`] method
...
#[async_trait::async_trait]
pub trait FinalitySlotHandlerWithRetry: FinalitySlotHandler {
    /// A method to be called with the event information,
    ...
    async fn handle_slot_with_retry(
        &self,
        store: Arc<Self::Store>,
        block: Block<TxHash>,
        ...
        let wrapped_task = || {
            self.handle_slot(store.clone(), block.clone())
...
impl<T> FinalitySlotHandlerWithRetry for T where T: FinalitySlotHandler + ?Sized {}

pub type FinalitySlotHandlerFor<W> = Box<
    dyn FinalitySlotHandler<Store = <W as BlockPoller>::Store> + Send + Sync,
>;
...
pub trait BlockPoller {
    ...
    async fn run(
        ...
        handlers: Vec<FinalitySlotHandlerFor<Self>>,
    ) -> crate::Result<()> {
        ...
        let task = || async {
            ...
            loop {
                ...
                tracing::trace!("last block number: {}", listener_config.start_block.unwrap_or_default());
                    ...
                    match client.get_block(dest_block).await {
                        Ok(Some(block)) => {
                            ...
                            let tasks = handlers.iter().map(|handler| {
                                ...
                                handler.handle_slot_with_retry(
                                    store.clone(),
                                    block.clone(),
                                    backoff,
                                )
                ...
                tracing::trace!("Polled from #{} to #{}, should cooldown? ({})", block, dest_block, should_cooldown);
                if should_cooldown {
                    let duration = Duration::from_millis(listener_config.polling_interval);
                    tracing::trace!(
